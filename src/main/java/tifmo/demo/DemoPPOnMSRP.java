 ...
 */
public class DemoPPOnMSRP {
	public static void main(String[] args) {
		//ppdb-1.0-xl-all.gz 0.8 1
		if(args.length < 2) {
			System.out.println("USAGE: PPDB threshold MSRPFile.txt ");
			return;
		}
		String currentPath = "";
		try {
		...
		List<String> files = new ArrayList<>(Arrays.asList("msrp_train.xml", "msrp_test.xml"));
//		for(int i = 2; i < args.length - 1; i++)
//			files.add(args[i]);
//		List<String> chosens = Arrays.asList("mikolov","turian","lcs");
		List<String> chosens = Arrays.asList("mikolov");
//		List<String> types = Arrays.asList("raw");
...
//		doRaw(files);
	...
	public static void doRaw(List<String> files) {
		String currentPath = "";

		try {
			currentPath = new File("").getCanonicalPath();
			for (int i = 0; i < files.size(); i++) {
				Map<Integer, MSPair> originMSPairs = EnUtils.readPairsInMSRP(currentPath.concat("/resources/input/").concat(files.get(i)));
				Map<Integer, Pair> originPairs = new HashMap<>();
				for(Integer id : originMSPairs.keySet())
					originPairs.put(id, originMSPairs.get(id).convertToPair(id));
					String destinationFilePath = currentPath.concat("/resources/output/") + files.get(i).substring(0,files.get(i).lastIndexOf(".")).concat(".xml");
					EnUtils.writePairs(destinationFilePath, originPairs);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			...
			for (int i = 0; i < files.size(); i++) {
				...
				if(!runMode) {
					for (String type : types) {
						for (String chosen : chosens) {
							String destinationFilePath = currentPath.concat("/resources/output/") + type + "/" + chosen + "_" + type + "_" + files.get(i).substring(0,files.get(i).lastIndexOf(".")).concat(".xml");
		...
		try {
			...
			for (int i = 0; i < files.size(); i++) {
				...
				if(!runMode) {
					for (String type : types) {
						for (String chosen : chosens) {
							String destinationFilePath = currentPath.concat("/resources/output/") + type + "/ss_" + chosen + "_" + type + "_" + files.get(i).substring(0,files.get(i).lastIndexOf(".")).concat(".xml");
							EnUtils.writePairs(destinationFilePath, targetPairs.get(type).get(chosen));
						}
					}
				}
